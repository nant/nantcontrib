...
namespace NAnt.Contrib.Tasks.StarTeam 
{
	...
	public abstract class StarTeamTask : SourceForge.NAnt.Task
	{
		...
		///</summary>
		///<remarks>Required if <tt>URL</tt> is not set. If you wish to set all connection parameters at once set <see cref="url"/></remarks>
		...
		/// </summary>
		/// <remarks>Required if <see cref="url"/> is not set. If you wish to set all connection parameters at once set <see cref="url"/></remarks>
		...
		public virtual string url
		{
			get
			{
				return string.Format("{0}:{1}@{2}:{3}/{4}/{5}",_username,_password,_servername,_serverport,_projectname,_viewname);
			}		
			set 
			{	
